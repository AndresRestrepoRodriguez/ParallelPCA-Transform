...
# Lee los parametros de ejecucion del script
...
if m.shape[2]:
    print 'la imagen multiespectral tiene '+str(m.shape[2])+' bandas y tama o '+str(m.shape[0])+'x'+str(m.shape[1])
else:
    print 'la primera imagen no es multiespectral'

if len(p.shape) == 2:
    print 'la imagen Pancromatica tiene tama o '+str(p.shape[0])+'x'+str(p.shape[1])
else:
...
r_host = m_host[:,:,0]
r_host = r_host.astype(np.float32)
g_host = m_host[:,:,1]
g_host = g_host.astype(np.float32)
b_host = m[:,:,2]
b_host = b_host.astype(np.float32)
size_rgb = m.shape
# Convierte la pancromatica a float32
p1_host = p.astype(np.float32)


# Inicial el tiempo de ejecucion
start=time.time()
...
p_subs_split_nb = split(p1_host,BLOCK_SIZE,BLOCK_SIZE)
...
t = skimage.io.imsave('/home/nvera/andres/pca/'+output_fus+'.tif',fusioned_image, plugin='tifffile')

#Tiempo de ejecuci�n para la transformada de Brovey en GPU
tiempo = (end-start)
